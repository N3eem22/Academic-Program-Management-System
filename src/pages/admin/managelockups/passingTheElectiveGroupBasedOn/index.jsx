import React from 'react';
import { DataTable } from '../../../../components/dataTable';

function PassingTheElectiveGroupBasedOn() {
  const universityId = 1;
    const nameOfLU= 'passingTheElectiveGroup';
    const property = 'اجتياز المجموعة الاختيارية بناءا علي';
    

  return (
    <div className="App">
      <DataTable  apiUri={`https://localhost:7095/api/PassingTheElectiveGroupBasedOn?UniversityId=${universityId}`}
        apiUriPut = {(id,value) => `https://localhost:7095/api/PassingTheElectiveGroupBasedOn/${id}?updatedPassingTheElectiveGroup=${value}`}
        apiUriDelete={(id) => `https://localhost:7095/api/PassingTheElectiveGroupBasedOn/${id}`}
        apiUriPost={`https://localhost:7095/api/PassingTheElectiveGroupBasedOn`}
        nameOfLU={nameOfLU}
        property= {property}/>
       
    </div>
  );
}

PassingTheElectiveGroupBasedOn.displayName = "PassingTheElectiveGroupBasedOn";

PassingTheElectiveGroupBasedOn.propTypes = {};

export { PassingTheElectiveGroupBasedOn };